...
function pick(raw: Raw, ...keys: string[]): unknown {
  ...
  for (const key of keys) {
    ...
    const match = entries.find(
      ([rawKey, value]) =>
        value != null && normalizeKey(rawKey) === normalizedKey,
    );
...
export function normalizeDashboardResumo(raw: unknown): DashboardResumoDTO {
  ...
  const totalVendas = num(
    pick(item, "totalVendas", "TotalVendas", "vendaMesAtual", "VendaMesAtual"),
  );
  return {
    totalVendas,
    totalVendasAnterior: num(
      pick(item, "totalVendasAnterior", "TotalVendasAnterior"),
    ),

    lucroMesAnoAnterior: num(
      pick(
        item,
        "lucroMesAnoAnterior",
        "LucroMesAnoAnterior",
        "Lucro do Mês do Ano Anterior",
      ),
    ),
    lucroMesAtual: num(
      pick(item, "lucroMesAtual", "LucroMesAtual", "Lucro do Mês Atual"),
    ),
    variacaoLucroMesAnoAnterior: num(
      pick(
        item,
        "variacaoLucroMesAnoAnterior",
        "VariacaoLucroMesAnoAnterior",
        "Variação do Lucro do Mês do Ano Anterior",
      ),
    ),
    variacaoVendas: num(pick(item, "variacaoVendas", "VariacaoVendas")),
    ticketMedio: num(pick(item, "ticketMedio", "TicketMedio", "Ticket Médio")),
    ticketMedioAnterior: num(
      pick(item, "ticketMedioAnterior", "TicketMedioAnterior"),
    ),
    variacaoTicketMedio: num(
      pick(item, "variacaoTicketMedio", "VariacaoTicketMedio"),
    ),
    lucroTotal: num(pick(item, "lucroTotal", "LucroTotal", "lucro", "Lucro")),
    lucroTotalAnterior: num(
      pick(
        item,
        "lucroTotalAnterior",
        "LucroTotalAnterior",
    ...
    variacaoLucro: num(pick(item, "variacaoLucro", "VariacaoLucro")),
    margemLucro: num(
      pick(item, "margemLucro", "MargemLucro", "Margem de Lucro"),
    ),
    margemLucroAnterior: num(
      pick(
        item,
        "margemLucroAnterior",
        "MargemLucroAnterior",
    ...
    variacaoMargemLucro: num(
      pick(item, "variacaoMargemLucro", "VariacaoMargemLucro"),
    ),
    totalDesconto: num(
      pick(item, "totalDesconto", "TotalDesconto", "Total de Descontos"),
    ),
    totalDescontoAnterior: num(
      pick(
        item,
        "totalDescontoAnterior",
        "TotalDescontoAnterior",
        "Total de Descontos Anterior",
      ),
    ),
    variacaoDesconto: num(pick(item, "variacaoDesconto", "VariacaoDesconto")),
    ...
    quantidadeVendidaAnterior: num(
      pick(
        item,
        "quantidadeVendidaAnterior",
        "QuantidadeVendidaAnterior",
        "Quantidade Vendida Anterior",
      ),
    ),
    variacaoQuantidadeVendida: num(
      pick(item, "variacaoQuantidadeVendida", "VariacaoQuantidadeVendida"),
    ),
    projecaoMes: num(
      pick(
        item,
    ...
    quantidadeVendasPeriodoAnterior: num(
      pick(
        item,
        "quantidadeVendasPeriodoAnterior",
        "QuantidadeVendasPeriodoAnterior",
        "quantidadeVendasAnterior",
        "QuantidadeVendasAnterior",
        "quantidadePedidosAnterior",
        "QuantidadePedidosAnterior",
      ),
    ),
    variacaoQuantidadeVendas: num(
      pick(item, "variacaoQuantidadeVendas", "VariacaoQuantidadeVendas"),
    ),
    pecasPorVenda: num(pick(item, "pecasPorVenda", "PecasPorVenda")),
    pecasPorVendaAnterior: num(
      pick(item, "pecasPorVendaAnterior", "PecasPorVendaAnterior"),
    ),
    variacaoPecasPorVenda: num(
      pick(item, "variacaoPecasPorVenda", "VariacaoPecasPorVenda"),
    ),
    vendaMesAnoAnteriorCompleto: num(
      pick(
        item,
        "vendaMesAnoAnteriorCompleto",
        "VendaMesAnoAnteriorCompleto",
        "Venda do Mês do Ano Anterior",
      ),
    ),
    vendaMesmoPeriodoAnoAnterior: num(
      pick(
        item,
        "vendaMesmoPeriodoAnoAnterior",
        "VendaMesmoPeriodoAnoAnterior",
        "vendasMesmoPeriodoAnoAnterior",
        "VendasMesmoPeriodoAnoAnterior",
      ),
    ),
    variacaoMesAnoAnterior: num(
      pick(item, "variacaoMesAnoAnterior", "VariacaoMesAnoAnterior"),
    ),

    variacaoMesmoPeriodoAnoAnterior: num(
      pick(
        item,
        "variacaoMesmoPeriodoAnoAnterior",
        "VariacaoMesmoPeriodoAnoAnterior",
      ),
    ),
    vendaMesAnoAnterior: num(
      pick(item, "vendaMesAnoAnterior", "VendaMesAnoAnterior"),
    ),
    variacaoProjecaoVsAnoAnterior: num(
      pick(
        item,
        "variacaoProjecaoVsAnoAnterior",
        "VariacaoProjecaoVsAnoAnterior",
      ),
    ),
    vendaMesAtual:
...
export function normalizeParticipacaoLojaItem(raw: Raw): ParticipacaoLojaDTO {
  return {
    ...
    valorVenda: num(pick(raw, "valorVenda", "ValorVenda", "valor", "Valor")),
    percentual: num(pick(raw, "percentual", "Percentual")),
  };
}
...
export function normalizeRegistros(
  items: unknown,
): RegistroVendasProdutosDTO[] {
  if (!Array.isArray(items)) return [];
